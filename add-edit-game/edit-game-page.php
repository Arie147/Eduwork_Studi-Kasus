...
$games = mysqli_query($conn, "SELECT * FROM `game` WHERE `id` = $id;");
...
<body>
  ...
  <div class="container-lg mb-5">
    <div class="row mb-3">
      <h2 class="text-light">Edit Game</h2>
    </div>
    <form action="../backend/edit/edit-game.php" method="post" enctype="multipart/form-data">
      <div>
      ...
      <div class="col-lg-6 mb-3">
        ...
        <input type="text" class="form-control" id="title" name="title" value="<?= $title; ?>" oninput="checkForm()">
      </div>
        ...
        <select class="form-select" id="developer" name="developer">
          <?php foreach ($developers as $developer) { ?>
            <option value="<?= $developer['developer_id'] ?>" 
              <?php if ($developer['developer_id'] == $developer_id) {
                      echo "selected";
        ...
        <select class="form-select" id="publisher" name="publisher">
          <?php foreach ($publishers as $publisher) { ?>
            <option value="<?= $publisher['publisher_id'] ?>" 
              <?php if ($publisher['publisher_id'] == $publisher_id) {
                      echo "selected";
      ...
      <div class="col-lg-2 mb-3">
        ...
        <input type="date" class="form-control" id="rls_date" name="rls_date" value="<?= $rls_date; ?>" oninput="checkForm()">
      </div>

      <div class="col-lg-2 mb-3">
        <label for="price" class="form-label text-light">Price</label>
        <input type="number" class="form-control" id="price" name="price" value="<?= $price; ?>" oninput="checkForm()">
      </div>

      <div class="col-lg-3 mb-4">
        <label for="newPicture" class="form-label text-light">New Picture</label>
        <input type="file" class="form-control" id="newPicture" name="newPicture">
        <p class="text-light mt-1">Max picture size is 3 MB</p>
      </div>

      <div class="col-lg-3 mb-4">
        <img src="../img/<?= $picture ?>" width="200" height="100">
        <input type="hidden" class="form-control" id="picture" name="picture" value="<?= $picture; ?>">
      </div>
